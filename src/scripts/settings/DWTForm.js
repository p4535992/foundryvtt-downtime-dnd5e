    ...
    this.activity.roll.forEach((roll) => {
      if (roll.type === "CUSTOM") {
        ...
        if (game.system.id === "dnd5e") {
          ...
          hdVals = this.actor.data.items.filter((item) => item.type === "class").map((hd) => parseInt(hd.data.data.hd));
        }
          ...
          custom.split(" ").forEach((formu) => {
            ...
            if (formu.startsWith("@")) {
              ...
              if (getProperty(context, testProp) === undefined) {
                warn("Downtime Tracking | " + formu + " is not present in the context.", true);
                fail = true;
              }
            }
          });
          if (fail) {
            throw error("Error in context for rolling.", true);
          }
        } else {
          warn("Downtime Tracking | This is not a valid roll formula.", true);
          return;
        }
      }
    });
  ...
  */
  ...
  async _updateObject(event, formData) {
    ...
    try {
      ...
      throw error("Downtime Tracking | Broken custom formula. Please fix.", false, e);
    }
    ...
    // local scope
    if (!this.world) {
      let flags = actor.getFlag(CONSTANTS.MODULE_ID, CONSTANTS.FLAGS.trainingItems);
      ...
      await actor.unsetFlag(CONSTANTS.MODULE_ID, CONSTANTS.FLAGS.trainingItems);
      await actor.setFlag(CONSTANTS.MODULE_ID, CONSTANTS.FLAGS.trainingItems, flags);
    }
    // World scope
    else {
